...
def login():
    ...
    if st.button("Login"):
        ...
        if role:
            ...
            if role in ["Admin", "System Administrator"]:  
...
def signup():
    ...
    with st.form("signup_form"):
        ...
        confirm_password = st.text_input(label="Confirmpassword", placeholder="Confirm your password", type="password", label_visibility="collapsed")
    ...
    if submitted:
        if password != confirm_password:
            ...
            if user:
                ...
                st.error("No account found for this email. Contact admin.")
            conn.close()
    ...
    if submitted:
        ...
        try:
            cursor.execute('''
                ...
                VALUES (?, ?, ?, ?, ?)
            ''', (email, username, hashed_password, role, 0)) 
            conn.commit()
            st.success(f"User {username} created successfully. Inactive until signup is completed.")
...
def admin():
    ...
    if 'role' not in st.session_state or st.session_state['role'] not in ["Admin", "System Administrator"]:
...
if 'current_page' not in st.session_state:
    st.session_state['current_page'] = "Menu"  # Default page before login
...
if 'is_logged_in' in st.session_state and st.session_state['is_logged_in']:
    ...
    if st.session_state['role'] in ["Admin", "System Administrator", "Program Director", "Academic Advisor"]:
        page_names_to_funcs = {
            "My Home": admin,
            "Create User": create_user if st.session_state['role'] == "Admin" else None,# Only for admins
...
page_names_to_funcs = {k: v for k, v in page_names_to_funcs.items() if v is not None}
demo_name = st.sidebar.selectbox(label="Choose page", options=list(page_names_to_funcs.keys()), label_visibility="collapsed")
st.session_state['current_page'] = demo_name
